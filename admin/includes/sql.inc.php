...
class sql_dummy{
 ...
 function connect($db,$host="localhost",$user="kost",$pass="otof"){dummy('connect');}
...
class sql_mysql extends sql_dummy {
 ...
 function connect($db,$host="localhost",$user="foto",$pass="otof"){
 ...
 function get_array($index=-1){
  $this->cur_col=0;
  if($index>-1)mysql_data_seek($this->cursor,$index);
  if($this->row=mysql_fetch_array($this->cursor))return true;
  return false;
 }
...
function db_conn($t=''){
 ...
 load_driver('mysql');
 ${'sql'.$t}=new sql_mysql();
 ${'sql'.$t}->connect($mysql['db'],$mysql['host'],$mysql['user'],$mysql['pass']);
