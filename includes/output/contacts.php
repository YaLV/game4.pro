...
if($_GET['x']=='help' || $_GET['x']=='help2') {
  $bdg="  HELP ";
} else {
  $bdg=$message->text("menu_contacts");
}
if($_GET['x']=='help') {
  $sql->query("select id,txt_$language from disclaimers where id='help'");
} elseif($_GET['x']=='help2') {
  $sql->query("select id,txt_$language from disclaimers where id='help2'");
} else {
  $sql->query("select id,txt_$language from disclaimers where id='contacts'");
}
$sql->get_row();
$h=$sql->get_col()."_hdr";
